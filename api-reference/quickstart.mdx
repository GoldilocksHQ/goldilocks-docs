...
1. Log in to the [Goldilocks web application](https://www.goldilocksai.app)
2. Navigate to the API Keys section within user profile section
...
API keys can have the following scopes (no user configuration required):
...
- `jobs:read` - Stream job progress via Server-Sent Events (SSE)
...
4. **Retrieve results** once the job completes using the [Retrieve Profiles endpoint](/api-reference/searches/profiles)
